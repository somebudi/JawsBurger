...
function renderMoment() {
    ...
    if (momentData.length === 0) {
        ...
        container.innerHTML =
        '<div class="no-results">Ora ono moment nang kene 📸</div>';
    ...
    container.innerHTML = momentData.map((item,index)=>{
        ...
        const safeImageUrl =
        fixDriveImageUrl(item.image_url);

        let sizeClass = '';

        if(index % 7 === 0){

            sizeClass = 'moment-big';
        ...
        return `
            <div class="moment-card ${sizeClass}">
                <img 
                    ...
                    onerror="
                    this.src='https://placehold.co/400x300/FFD700/3E0B0E?text=Jaws+Moment'
                    "
                >
            </div>
        `;
